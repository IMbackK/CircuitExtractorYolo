...
def main():
    ...
    parser.add_argument('--data', type=str, default='../data/graphType/labeled',
                        help='data directory')
    parser.add_argument('--valid', type=str, default='../data/graphType/test',
    ...
    if args.save_model:
        dummy_data = torch.randn(dataset1.__getitem__(0)[0].size())
        dummy_data = dummy_data.repeat(10, 1, 1, 1)
        input_names = ["input"];
        output_names = ["output"];
        torch.save(model.state_dict(), "out.pt")
        torch.onnx.export(model, dummy_data, "out.onnx", input_names=input_names, output_names=output_names)
